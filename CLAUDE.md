...
- **Company voice, NOT a one-man band (per Tal, 2026-07-09).** DealMentor is a company/product, not "rent Tal." Do **not** write product copy in Tal's first person ("I built this", "I can't be on every deal", "I charge $20K/mo", "most teams can't afford me", "with me on call") - that makes us sound like a solo consultancy. Tal is the **founder** and a live fractional CRO **is** included, but the live CRO is a **supporting feature, not the identity** - keep it out of heroes/keystones as the lead. Product copy speaks as the company/product ("DealMentor reads every deal…"); founder credibility is fine in **third person / company voice** ("built by senior sales operators", "our founder ran this playbook for two decades"). Founder-story first person is fine **only** on `/about` (that's its job).
- Keystone claim: *a CRM with a real CRO inside - it reads every deal and hands back the next move.* Twenty AI tools + a live fractional CRO sit around that spine (live CRO = a benefit, not the headline).
- `/index` has a founder quote (company-vision voice, "we built DealMentor so every rep gets that judgment…") - keep it founder-as-company-founder, never "rent-me" solo-operator framing.
